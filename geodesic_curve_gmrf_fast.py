...
#################################################################################################
# Generates an output of a pairwise isotropic Gaussian-Markov random field with Numba (parallel)
#################################################################################################
@njit(parallel=True)
def mcmc_parallel(mean, variance, beta, size, max_it):
    ...
    for _ in range(max_it):
        amostras = np.zeros(((n-2)*(n-2), 9))
        sample_tmp = padded.copy()

        for i in prange(1, n-1):  # paraleliza sobre linhas
            for j in range(1, n-1):
                ind = (i-1)*(n-2) + (j-1)
                neigh = padded[i-1:i+2, j-1:j+2].flatten()
                amostras[ind, :] = neigh
                vizinhanca = np.concatenate((neigh[:4], neigh[5:]))
                central = neigh[4]
                P1 = (1 / (np.sqrt(2*np.pi)*sigma)) * np.exp((-1/(2*sigma**2)) * (central - mean - beta*np.sum(vizinhanca - mean))**2)
                g = np.random.normal(mean, sigma)
                while g < mean - 3*sigma or g > mean + 3*sigma:
                    g = np.random.normal(mean, sigma)
                P2 = (1 / (np.sqrt(2*np.pi)*sigma)) * np.exp((-1/(2*sigma**2)) * (g - mean - beta*np.sum(vizinhanca - mean))**2)
                limiar = min(1, P2/P1)
                if np.random.rand() <= limiar:
                    sample_tmp[i, j] = g

        padded = sample_tmp.copy()

    nucleo = padded[1:-1, 1:-1]
...
def metric_tensor(samples, mean, variance, beta):  
	...
	# Adicionei isso
	rho_sum = rho.sum()
	sigma_minus_sum = sigma_minus.sum()
	# g_11
	...
	g = tensor + 0.001*np.eye(3)
	#g = tensor
	# Computes the entropy
	entropy = 0.5*(np.log(2*np.pi*variance) + 1) - (1/(variance))*( beta*rho.sum() - ((beta**2)/2)*sigma_minus.sum() )

	return g, entropy, rho_sum, sigma_minus_sum

#########################################################################################################
# Computes the second fundamental form of the parametric space (second-order Fisher information matrix)
#########################################################################################################
def second_fundamental_form(samples, mean, variance, beta):  
	# Metric tensor
	tensor = np.zeros((3,3))
	# number of neighbors
	delta = 8
	...
	rho = np.concatenate((left_half, right_half))
	# g_11
	g_11 = (1/variance)*((1 - beta*delta)**2)
	tensor[0,0] = g_11
	# g_12 = g_21 = 0
	# g_13 = g_31 = 0
	# g_22
	g_22 = (1/(2*variance**2)) - (1/variance**3)*(2*beta*rho.sum() - (beta**2)*sigma_minus.sum())
	tensor[1,1] = g_22
	# g_23 = g_32
	g_23 = (1/variance**2)*(rho.sum() - beta*sigma_minus.sum())
	g_32 = g_23
	tensor[1,2] = g_23
	tensor[2,1] = g_32
	# g_33
	g_33 = (1/variance)*sigma_minus.sum()
	tensor[2,2] = g_33
	# Regularization to avoid numerical issues (0.01)
	g = tensor + 0.001*np.eye(3)
	#g = tensor
...
def RK4(a, b, n, mean, variance, beta, alpha1, alpha2, alpha3):
	...
	rho_sum_vector = np.zeros(n)
	sigma_minus_sum_vector = np.zeros(n)
	...
	for i in range(n):
		...
		#output, samples = mcmc_fast(mean, variance, beta, SIZE, MAX_IT)
		output, samples = mcmc_parallel(mean, variance, beta, SIZE, MAX_IT)
		# Save image file
		if i == 0:
		...
		g, entropy, rho_sum, sigma_minus_sum = metric_tensor(samples, mean, variance, beta)
		rho_sum_vector[i] = rho_sum
		sigma_minus_sum_vector[i] = sigma_minus_sum
		second_form = second_fundamental_form(samples, mean, variance, beta)
		entropy_vector[i] = entropy
		shape_operator = np.dot(-second_form, np.linalg.inv(g))
		curvature_vector[i] = np.linalg.det(shape_operator)
		...
		print('Curvature = %f' %curvature_vector[i])
		print()
	...
	return (mean_vector, variance_vector, beta_vector, alpha1_vector, alpha2_vector, alpha3_vector, geodesic_vector, entropy_vector, curvature_vector, rho_sum_vector, sigma_minus_sum_vector)
...
if __name__ == '__main__':
	...
	reverse = True		# If True the time-reverse process will be performed
	a = 0 				# initial time
	b = 10 				# final time
	...
	ax.plot(means, variances, betas, 'b', label='Original')
	...
	if reverse:
		...
		curvatures_reverse = LR[8]
		rhos_reverse = LR[9]
		sigma_minus_reverse = LR[10]
		# Initial and final points
		...
		print()
		# Plot geodesic curve
		ax.plot(means_reverse, variances_reverse, betas_reverse, 'r', label='Reverse')
		entropies = np.concatenate((entropies, entropies_reverse))
		curvatures = np.concatenate((curvatures, curvatures_reverse))
		rhos = np.concatenate((rhos, rhos_reverse))
		sigma_minus = np.concatenate((sigma_minus, sigma_minus_reverse))
	fim = time.time()
	print()
	print('Elapsed time: %f' %(fim-ini))	
	# Show the graph in the screen
	ax.set_xlabel('Mean')
	ax.set_ylabel('Variance')
	ax.set_zlabel('Beta', rotation=90)
	#ax.legend()
	plt.show()
	# Plot entropy
	plt.figure(2)
